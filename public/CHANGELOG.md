...
按方案 A 收窄词缀例词功能，回归简单背单词主线。

### 🧭 调整

- 词缀例词不再自动批量收入 195 词的大例词本。
- 只有用户在例词旁主动点「＋标签」并打标签后，新词才会保存到“我标记的例词”。
- 已存在于初中 / 高中 / 四级 / 六级 / 专业词库的例词不重复入库，标签写回原词库。
- 全局页更名为「我标记的例词」，只显示主动标记过的例词。
- 词缀页和首页入口改为轻量统计，不再强调自动收集的新词数量。
- 历史备份中未标记的自动收集词会被清理，避免旧数据继续撑大词缀例词本。
- 保留模糊 / 重点 / 必背例词的专用复习入口。

### 🔧 工程化

- `APP_VERSION` 升到 `v1.2.3`
- Service Worker `CACHE_VERSION` 升到 `v1.2.3`
...
- 修复直接停留在 `#/affix-batch` 刷新后，点击 `10 / 20 / 50 / 100 / 全部` 没反应的问题。
- 原因是本轮词缀列表刷新后为空，页面降级显示词缀入口，但目标 hash 没变化，导致路由不刷新。

### 🔧 工程化

- `APP_VERSION` 升到 `v1.2.2`
- Service Worker `CACHE_VERSION` 升到 `v1.2.2`

## [v1.2.1] - 2026-09-26

例词支持在词缀卡片内直接打标签，并接入复习链路。

### ✨ 新增

- 词缀卡片的每个例词旁新增「＋标签」入口，点开后可选择：认识 / 模糊 / 重点 / 必背 / 过关。
- 已标记的例词会直接显示当前标签，例如「重点」。
- 例词标签支持单独撤销/重新标记；标记结果立即保存。
- 已存在于主词库的例词，标签会写回原词库，并参与原词库的普通复习和加权抽查。
- 不在主词库的新词会保存在全局词缀例词本，并新增「复习模糊 / 重点 / 必背」入口。

### 🔧 优化

- 建立主词库单词索引，减少例词状态判断的重复扫描。
- 备份导入导出继续包含词缀例词数据。

...
- 词缀批量页新增「词缀模式 / 例词模式」切换。
- **例词模式**会把本轮选中的 10/20/50/100/全部词缀的例词一次性摊平列出，支持单词级勾选。
- 例词条目支持单独发音、查看释义、来源词缀、主词库来源和当前标签。
- 新增全局 `#/affix-words` **词缀例词本**，自动收集词缀练习中出现的新词。
- 新词写入档案内的全局词缀例词本；已存在于初中 / 高中 / 四级 / 六级 / 专业词库的例词不重复入库，只显示来源。
- 对已有主词库例词打标签时，标签会写回原词库记录。
- 全局例词本支持「全部 / 新词 / 已在主词库 / 未标记 / 已标记 / 已掌握」筛选，并支持批量标记。
- 词缀页新增词缀例词本入口；首页词缀卡片显示例词新词数量，不增加独立首页大卡。

### 🔧 工程化

- `APP_VERSION` 升到 `v1.2.0`
- Service Worker `CACHE_VERSION` 升到 `v1.2.0`
- 备份导入导出补齐词缀学习数据与 `aWords` 全局例词数据。
...
- 更新日志整体统一为**版本号从新到旧**排列：`v1.1.6 → v1.1.5 → … → v1.0.0`。
- 不再混用“旧版本在前”和“新版本在前”的排序方式。

### 🔧 工程化

- `APP_VERSION` 升到 `v1.1.6`
- Service Worker `CACHE_VERSION` 升到 `v1.1.6`
---
